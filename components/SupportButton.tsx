  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        {/* <button
          type="button"
          className="p-0 text-sm text-red-600 bg-transparent transition-colors hover:text-red-500 dark:text-red-400 dark:hover:text-red-300 hover:underline"
        >
          支援开发者
        </button> */}
      </DialogTrigger>
        ...
        {wechatPayQrUrl ? (
          <div className="flex justify-center mt-3">
            ...
            <img
              src={wechatPayQrSrc}
              alt="微信赞赏码"
              className="object-contain w-48 h-48 rounded-lg border border-border md:h-60 md:w-60"
              onError={() => {
            ...
            {donationAcknowledgements.map((item, index) => (
              ...
              >
                <div className="flex gap-3 justify-between items-start">
                  ...
                  <p className="text-sm font-bold shrink-0 text-foreground">¥{item.amount}</p>
                </div>
                {item.message ? (
                  <p className="mt-2 text-xs leading-5 break-words text-muted-foreground">{item.message}</p>
                ) : null}
              </article>
            ))}
          </div>

          <div className="hidden overflow-hidden mt-3 rounded-lg border border-border md:block">
            <table className="w-full text-xs text-left table-fixed text-muted-foreground">
              <thead className="bg-muted text-[11px] font-semibold text-muted-foreground">
                <tr>
                  <th className="px-3 py-2 w-20">打赏日期</th>
                  <th className="px-3 py-2 w-32">打赏人</th>
                  <th className="px-3 py-2 w-20">打赏金额</th>
              ...
              <tbody>
                {donationAcknowledgements.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2 font-semibold align-top whitespace-nowrap">
                      {item.amount}
                    </td>
