...
const SEARCH_RATE_LIMIT_MAX_REQUESTS = 10;
...
type SearchItems = Awaited<ReturnType<typeof searchBggBoardgames>>;
...
async function getCachedSearchItems(query: string, kind: SubjectKind): Promise<SearchItems> {
  ...
  const requestPromise = searchBggBoardgames({ query, kind });
...
export async function handleBggSearchRequest(
  request: Request,
  options?: {
